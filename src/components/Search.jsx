...
export default function Search({ peliculas }) {
    ...
    function buscarPeliculas(consulta, peliculas) {
        ...
        return peliculas.filter((pelicula) => {
            ...
            const actores = pelicula.actores?.join(' ').toLowerCase() || '';

            return (
                titulo.includes(consultaMinusculas) ||
                director.includes(consultaMinusculas) ||
                actores.includes(consultaMinusculas)
            );
        });
    }
    ...
    return (
        <div className="relative">
            ...
            {consulta && (
                <div className="absolute z-50 mt-2 bg-neutral-900 text-white rounded-xl w-72 shadow-lg overflow-y-auto max-h-80">
                    {resultados.length > 0 ? (
                        ...
                        <div className="px-4 py-2 text-sm text-neutral-400">No se encontraron resultados.</div>
                    )}
                </div>
            )}
        </div>
